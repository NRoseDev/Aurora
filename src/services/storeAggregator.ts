...
interface StoreAggregatorConfigs {
  walmart?: {
    clientId: string;
    clientSecret: string;
  };
...
export class StoreAggregator {
  ...
  constructor(config: StoreAggregatorConfigs) {
    if (config.walmart) {
  ...
  public async fetchAllProducts(): Promise<UnifiedProduct[]> {
    ...
    if (this.walmartService) {
      promises.push(
        this.walmartService.fetchListings().catch((err) => {
    ...
    if (this.ebayService) {
      promises.push(
        this.ebayService.fetchListings().catch((err) => {
    ...
    if (this.wooService) {
      promises.push(
        this.wooService.fetchListings().catch((err) => {
...
export async function fetchAllProducts(
  configs: StoreAggregatorConfigs
