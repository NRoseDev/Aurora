...
export default function Storefront() {
  ...
  const [affiliateResources] = useState([
    {
      id: 1,
      title: "Recommended Creator Resource",
      description: "A resource selected to complement this creator's content.",
      url: "#affiliate-resource-1",
      disclosure: "Affiliate recommendation"
    },
    {
      id: 2,
      title: "Tools & Resources",
      description: "Helpful products and services related to this creator's work.",
      url: "#affiliate-resource-2",
      disclosure: "Affiliate recommendation"
    }
  ]);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('keydown', handleHardwareDeviceInput);
  }, [inputMode]);
  ...
  const handlePurchase = (product) => {
    const sizeInfo =
      product.type === 'apparel'
        ? ` (Size: ${selectedSizes[product.id] || 'Not Selected'})`
        : '';

    alert(
      `Redirecting to secure Stripe checkout for: ${product.name}${sizeInfo}`
    );
  };

  const handleAffiliateClick = (resource) => {
    console.log(
      `Affiliate resource selected: ${resource.title}`
    );
        ...
        >
          <label>
            <input
              ...
              onChange={(e) =>
                setIsDyslexiaFont(e.target.checked)
              }
            />{' '}
            Dyslexia Font
          </label>
