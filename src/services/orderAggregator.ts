...
export const fetchAllOrders = async (configs: OrderAggregatorConfigs): Promise<UnifiedOrder[]> => {
  const orderPromises: Promise<UnifiedOrder[]>[] = [];

  // Shopify Orders
  if (configs.shopify) {
    orderPromises.push(
      fetch(`https://${configs.shopify.shopDomain}/admin/api/2024-01/orders.json?status=any`, {
        headers: {
        ...
        .then(async (res) => {
          ...
          return data.orders.map((order: any): UnifiedOrder => ({
  ...
  // WooCommerce Orders
  if (configs.woo) {
    const credentials = btoa(`${configs.woo.consumerKey}:${configs.woo.consumerSecret}`);
    orderPromises.push(
      fetch(`${configs.woo.siteUrl}/wp-json/wc/v3/orders`, {
        ...
        .then(async (res) => {
          ...
          const data = await res.json();
          return data.map((order: any): UnifiedOrder => ({
            id: order.id.toString(),
